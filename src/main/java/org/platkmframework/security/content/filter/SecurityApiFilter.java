    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            if (StringUtils.isNotBlank(req.getPathInfo())) {
                ...
                } else if (req.getPathInfo().endsWith(SessionConstants.C_SING_UP_PATH)) {
                    List<?> list = ObjectContainer.instance().getListObjectByInstance(SingUpSecurityHandler.class);
                    if (list == null || list.isEmpty())
                        throw new ServletException("Debe crear una implementación de la interface " + SingUpSecurityHandler.class.getName());
                    SingUpSecurityHandler singUpSecurityHandler = (SingUpSecurityHandler) list.get(0);
                    singUpSecurityHandler.signUp(req, resp);
